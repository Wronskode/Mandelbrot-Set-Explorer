...
pub fn main() {
    ...
    'running: loop {
        for event in event_pump.poll_iter() {
            match event {
                ...
                | Event::MouseButtonDown { x, y, .. } => {
                    modified = true;
                    canvas.clear();
                    let result = compute_zoom(width, height, x as f64, y as f64, x_min, x_max, y_min, y_max, i);
                    x_min = result.0;
                    x_max = result.1;
                    y_min = result.2;
                    y_max = result.3;
        ...
        if video {
            canvas.clear();
            s += 0.0001;
            let result = compute_zoom(width, height, x_target, y_target, x_min, x_max, y_min, y_max, s);
            x_min = result.0;
            x_max = result.1;
            y_min = result.2;
            y_max = result.3;
...
fn compute_zoom(width: f64, height: f64, x: f64, y: f64, mut x_min: f64, mut x_max: f64, mut y_min: f64, mut y_max: f64, zoom_ratio: f64) -> (f64, f64, f64, f64, f64, f64) {
